...
export const LessonFooter = ({ footerContent, onCheck, canCheck, status }: LessonFooterProps) => {
  if (status === "completed") {
    return (
      <div className='fixed bottom-0 left-0 right-0 bg-gradient-to-r from-green-400 to-blue-500 text-white p-8'>
        <div className='max-w-4xl mx-auto text-center space-y-4'>
          <div className='text-6xl'>
            <Coins
              className={cn("w-16 h-16 mx-auto animate-bounce", GAME_ELEMENT_COLORS.XP.text)}
            />
          </div>
  ...
  return (
    <div className='fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-900 border-t dark:border-gray-700 p-4'>
      <div className='max-w-4xl mx-auto flex justify-end'>
        <Button onClick={onCheck} disabled={!canCheck} size='lg' className='min-w-[120px]'>
          CHECK
        </Button>
      </div>
    </div>
