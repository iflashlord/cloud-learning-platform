...
export const LessonFooter = ({
      ...
      <div className='fixed bottom-0 left-0 right-0 bg-gradient-to-r from-green-400 to-blue-500 text-white p-8'>
        <div className='max-w-4xl mx-auto text-center space-y-4'>
          <div className='text-6xl'>
            <Coins className='w-16 h-16 mx-auto animate-bounce' />
          </div>
          <h2 className='text-3xl font-bold'>Lesson Complete!</h2>
          <p className='text-lg opacity-90'>
            Great job! You&apos;ve successfully completed this AWS fundamentals
            lesson.
          </p>
  ...
  if (footerContent) {
    return (
      <div
        className={`fixed bottom-0 left-0 right-0 border-t-2 ${footerContent.bgClass} p-4`}
      >
        <div className='max-w-4xl mx-auto flex items-center justify-between'>
          <div className='flex items-center space-x-3'>
            {footerContent.button.icon}
            <span
              className={`text-lg font-semibold ${footerContent.textClass}`}
            >
          ...
          <Button
            ...
            className={`min-w-[120px] ${
              footerContent.button.animate ? "animate-pulse" : ""
            }`}
          >
