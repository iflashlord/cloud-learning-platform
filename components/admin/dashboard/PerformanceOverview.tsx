...
export const PerformanceOverview = ({
  ...
  const avgQuestionsPerLesson = challenges > 0 && lessons > 0 
    ? Math.round(challenges / lessons * 100) / 100 
    : 0;
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {metrics.map((metric, index) => (
          <div key={index} className={`text-center p-6 ${metric.bgGradient} rounded-xl`}>
            ...
            <div className={`text-sm ${metric.color.replace('text-', 'text-').replace('-600', '-700')} font-medium`}>
              {metric.label}
            </div>
