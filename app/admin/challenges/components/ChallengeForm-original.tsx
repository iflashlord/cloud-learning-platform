...
export const ChallengeForm = ({
    ...
    if (formData.lessonId) {
      try {
        const response = await fetch(
          `/api/challenges?lessonId=${formData.lessonId}`
        )
        if (response.ok) {
  ...
  const addOption = () => {
    const newOption: ChallengeOption = {
      ...
      guide: "",
      ...(formData.type === "DRAG_DROP" && {
        order: formData.challengeOptions.length + 1,
      }),
    ...
    try {
      ...
      if (formData.type === "TEXT_INPUT" || formData.type === "SPEECH_INPUT") {
        ...
        if (formData.type !== "DRAG_DROP") {
          const hasCorrectOption = formData.challengeOptions.some(
        ...
        const hasEmptyOption = formData.challengeOptions.some(
        ...
        if (formData.type === "IMAGE_SELECT") {
          const hasEmptyImage = formData.challengeOptions.some(
            (option) => !option.imageSrc?.trim()
          )
          if (hasEmptyImage) {
            toast.error(
              "Image selection questions require all options to have images"
            )
            return
          }
        }
        ...
        if (formData.type === "DRAG_DROP") {
          const orders = formData.challengeOptions.map(
            (option) => option.order || 0
          )
          const uniqueOrders = new Set(orders)
          if (
            orders.length !== uniqueOrders.size ||
            orders.some((o) => o < 1 || o > orders.length)
          ) {
            toast.error(
              "Drag & drop questions must have unique order values from 1 to " +
                orders.length
            )
            return
          }
    ...
    <div className='bg-white rounded-lg shadow p-6'>
      <form onSubmit={handleSubmit} className='space-y-6'>
        ...
        <div>
          ...
          >
            ...
            <span className='text-sm text-gray-500 ml-1'>
              - Helps users when they need guidance
            </span>
          </label>
          <textarea
            id='hint'
        ...
        <div>
          ...
          >
            <option value='SELECT'>
              Multiple Choice - Students pick from several options
            </option>
            <option value='ASSIST'>
              Fill in the Blank - Students complete a sentence
            </option>
            <option value='TRUE_FALSE'>
              True/False - Students choose true or false
            </option>
            <option value='DRAG_DROP'>
              Drag &amp; Drop - Students arrange items in order
            </option>
            <option value='TEXT_INPUT'>
              Text Input - Students type their answer
            </option>
            <option value='IMAGE_SELECT'>
              Image Selection - Students choose from images
            </option>
            <option value='LISTENING'>
              Listening - Students listen to audio and answer
            </option>
            <option value='SPEECH_INPUT'>
              Speech Input - Students answer using voice recognition
            </option>
            <option value='VIDEO'>
              Video Question - Students watch a video and answer
            </option>
          </select>
          ...
          <div className='mt-3 p-3 bg-blue-50 border border-blue-200 rounded-md'>
            <p className='text-sm text-blue-800'>
              {formData.type === "SELECT" && (
                <>
                  <strong>Multiple Choice:</strong> Students will see your
                  question and select one correct answer from multiple options.
                  Mark one option as correct. Great for testing knowledge and
                  concepts.
                </>
              )}
              {formData.type === "ASSIST" && (
                <>
                  <strong>Fill in the Blank:</strong> Students complete a
                  sentence by choosing the right word/phrase. Use &quot;__&quot;
                  in your question where the blank should appear. Perfect for
                  vocabulary and key terms.
                </>
              )}
              {formData.type === "TRUE_FALSE" && (
                <>
                  <strong>True/False:</strong> Students determine if a statement
                  is true or false. Mark the correct answer. Ideal for testing
                  facts and understanding of concepts.
                </>
              )}
              {formData.type === "DRAG_DROP" && (
                <>
                  <strong>Drag & Drop:</strong> Students arrange items in the
                  correct order by dragging and dropping. Set the correct order
                  position for each item (1, 2, 3, etc.). Great for sequences
                  and procedures.
                </>
              )}
              {(formData.type === "TEXT_INPUT" ||
                ...
                <>
                  <strong>Text Input:</strong> Students type their answer in a
                  text field. Set the correct answer below. Perfect for names,
                  definitions, and short answers.
                </>
              )}
              {formData.type === "IMAGE_SELECT" && (
                <>
                  <strong>Image Selection:</strong> Students choose the correct
                  image from multiple visual options. Each option needs an image
                  URL. Excellent for visual recognition and identification.
                </>
              )}
              {formData.type === "LISTENING" && (
                <>
                  <strong>Listening:</strong> Students listen to audio and
                  answer questions about what they heard. Requires an audio file
                  URL. Perfect for pronunciation, comprehension, and audio
                  content.
                </>
              )}
              {formData.type === "SPEECH_INPUT" && (
                <>
                  <strong>Speech Input:</strong> Students speak their answer
                  using voice recognition technology. Their speech is
                  transcribed to text and compared with the correct answer.
                  Ideal for pronunciation practice and oral responses.
                </>
              )}
              {formData.type === "VIDEO" && (
                <>
                  <strong>Video Question:</strong> Students watch a video and
                  then answer a question based on the content. Requires a video
                  file URL. Perfect for visual demonstrations, tutorials, and
        ...
        {formData.type === "LISTENING" && (
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setFormData((prev) => ({ ...prev, audioSrc: e.target.value }))
              }
              className='w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent'
        ...
        {formData.type === "VIDEO" && (
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setFormData((prev) => ({ ...prev, videoSrc: e.target.value }))
              }
              className='w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent'
        ...
        {(formData.type === "TEXT_INPUT" ||
          ...
          <div>
            ...
            >
              ...
              <span className='text-sm text-gray-500 ml-1'>
                - Expected user input (case-insensitive)
              </span>
            </label>
            <input
              type='text'
            ...
            <div>
              <div className='flex items-center justify-between mb-4'>
                <label className='block text-sm font-medium text-gray-700'>
                  ...
                  {formData.type === "DRAG_DROP" && (
                    <span className='text-sm text-gray-500 ml-1'>
                      - Items will be shuffled for drag & drop
                    </span>
                  )}
                </label>
                <Button
                    ...
                    <div className='space-y-3'>
                      ...
                      {formData.type !== "DRAG_DROP" && (
                        <div className='flex items-center'>
                          <input
                            type={
                              formData.type === "TRUE_FALSE" ? "radio" : "radio"
                            }
                            name='correctOption'
                      ...
                      <div>
                        ...
                        <textarea
                          value={option.guide || ""}
                          onChange={(e) =>
                            handleOptionChange(index, "guide", e.target.value)
                          }
