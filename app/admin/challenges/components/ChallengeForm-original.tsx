...
export const ChallengeForm = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.type === "TEXT_INPUT" || formData.type === "SPEECH_INPUT") {
        ...
        if (formData.type === "DRAG_DROP") {
          ...
          if (
            ...
            toast.error(
              "Drag & drop questions must have unique order values from 1 to " + orders.length,
            )
            return
          }
        }
      ...
      if (response.ok) {
        toast.success(
          challengeId ? "Challenge updated successfully" : "Challenge created successfully",
        )
          ...
          <div className='mt-3 p-3 bg-blue-50 border border-blue-200 rounded-md'>
            <p className='text-sm text-blue-800'>
              {formData.type === "SELECT" && (
                <>
                  <strong>Multiple Choice:</strong> Students will see your question and select one
                  correct answer from multiple options. Mark one option as correct. Great for
                  testing knowledge and concepts.
                </>
              )}
              {formData.type === "ASSIST" && (
                <>
                  <strong>Fill in the Blank:</strong> Students complete a sentence by choosing the
                  right word/phrase. Use &quot;__&quot; in your question where the blank should
                  appear. Perfect for vocabulary and key terms.
                </>
              )}
              ...
              {formData.type === "DRAG_DROP" && (
                <>
                  <strong>Drag & Drop:</strong> Students arrange items in the correct order by
                  dragging and dropping. Set the correct order position for each item (1, 2, 3,
                  etc.). Great for sequences and procedures.
                </>
              )}
              {(formData.type === "TEXT_INPUT" || formData.type === "SPEECH_INPUT") && (
                <>
                  <strong>Text Input:</strong> Students type their answer in a text field. Set the
                  correct answer below. Perfect for names, definitions, and short answers.
                </>
              )}
              {formData.type === "IMAGE_SELECT" && (
                <>
                  <strong>Image Selection:</strong> Students choose the correct image from multiple
                  visual options. Each option needs an image URL. Excellent for visual recognition
                  and identification.
                </>
              )}
              {formData.type === "LISTENING" && (
                <>
                  <strong>Listening:</strong> Students listen to audio and answer questions about
                  what they heard. Requires an audio file URL. Perfect for pronunciation,
                  comprehension, and audio content.
                </>
              )}
              {formData.type === "SPEECH_INPUT" && (
                <>
                  <strong>Speech Input:</strong> Students speak their answer using voice recognition
                  technology. Their speech is transcribed to text and compared with the correct
                  answer. Ideal for pronunciation practice and oral responses.
                </>
              )}
              {formData.type === "VIDEO" && (
                <>
                  <strong>Video Question:</strong> Students watch a video and then answer a question
                  based on the content. Requires a video file URL. Perfect for visual
                  demonstrations, tutorials, and multimedia-based assessments.
                </>
              )}
            </p>
          </div>
                    ...
                    {formData.type === "DRAG_DROP" && (
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleOptionChange(index, "order", parseInt(e.target.value))
                          }
                          min='1'
